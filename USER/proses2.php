...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_num_rows($cari) > 0) {
        ...
        if (mysqli_num_rows($ambil) > 0) {
            ...
            $sql = "INSERT INTO $tabel (id_respon,  pertanyaan_id, jenisKuisioner_id, judul_id, presepsi, harapan) VALUES ('$id','$id_pertanyaan','$jenis','$judul','$presepsi','$harapan') ";
    ...
    // $ambil = mysqli_query($koneksi, "SELECT * FROM tb_hasil WHERE pertanyaan_id = '$id_pertanyaan'");
    // if (mysqli_num_rows($ambil) > 0) {
    //     $update = mysqli_query($koneksi, "UPDATE tb_hasil SET presepsi = '$presepsi', harapan = '$harapan'  ");
    //     if ($update) {
    //         echo "success|Data Berhasil Ditambahkan";
    //     }
    // } else {
    //     $query = mysqli_query($koneksi, "INSERT INTO tb_hasil (id_hasil, pertanyaan_id, jenisKuisioner_id, judul_id, presepsi, harapan) VALUES ('$id','$id_pertanyaan','$jenis','$judul','$presepsi','$harapan')");
